...
def update_evaluation(request: UpdateEvaluationRequest):
    ...
    try:
        ...
        existing = get_user_evaluation(user_id, record_date=today) or {}
        ...
        save_user_evaluation(user_id, evaluation_data)

        # 同时更新 learning_records 作为实时快照（fallback 查询用）
        lr = get_learning_record(user_id) or {}
        ...
        save_learning_record(
            user_id=user_id,
            interaction_count=evaluation_data.get("interactionCount", _coerce_int(lr.get("interaction_count"))),
            code_practice_time=evaluation_data.get("codePracticeTime", _coerce_int(lr.get("code_practice_time"))),
            socratic_pass_rate=evaluation_data.get("socraticPassRate", _coerce_float(lr.get("socratic_pass_rate"))),
            difficulty_level=evaluation_data.get("difficultyLevel", lr.get("difficulty_level") or "basic"),
            profile_json=json.dumps(
                {
                    "focus_time_today": evaluation_data.get("focusTimeToday", _coerce_int(lr_profile.get("focus_time_today"))),
                    "flashcards_studied": evaluation_data.get("flashcardsStudied", _coerce_int(lr_profile.get("flashcards_studied"))),
                    "streak_days": evaluation_data.get("streakDays", _coerce_int(lr_profile.get("streak_days"))),
                },
                ensure_ascii=False,
            ),
        )
...
def get_evaluation(user_id: int):
    ...
    try:
        from db import get_user_evaluation, get_learning_record

        today = date.today().isoformat()
        ev = get_user_evaluation(user_id, record_date=today) or {}
        lr = get_learning_record(user_id) or {}
...
def get_evaluation_history(user_id: int, days: int = 30):
    """获取用户最近 N 天的评估指标历史"""
    try:
        from db import get_user_evaluation_history

        history = get_user_evaluation_history(user_id, days=days)
